...
export function Quiz() {
    ...
    async function getQuiz() {
        ...
        try {
            setError('');
            setLoading(true)
            const quizData = await axios.get<Quiz>('http://localhost:8080/api/v1/quiz/' + id, {headers: {"Authorization": `Bearer ${token}`}});
    ...
    async function sendAns() {
        ...
        try {
            setError('');
            setLoading(true)
            const resultData = await axios.post<Result[]>('http://localhost:8080/api/v1/quiz/' + id, selectedAnswers, {headers: {"Authorization": `Bearer ${token}`}});
            setQuiz(null)
